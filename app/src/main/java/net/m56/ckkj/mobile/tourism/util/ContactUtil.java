...
public class ContactUtil {
    ...
    public static void addContact(Context context, String name, String phone) {
        ...
        ContentProviderOperation op1 = ContentProviderOperation.newInsert(RawContacts.CONTENT_URI).withValues(values).build();
        operations.add(op1);
        ContentProviderOperation op2 = ContentProviderOperation.newInsert(Data.CONTENT_URI).withValueBackReference(Data.RAW_CONTACT_ID, 0).withValue(Data.MIMETYPE, ContactsContract.CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE).withValue(ContactsContract.CommonDataKinds.StructuredName.DISPLAY_NAME, name.trim()).build();
        operations.add(op2);
        for (int i = 0; i < ph.length; i++) {
            ContentProviderOperation op = ContentProviderOperation.newInsert(Data.CONTENT_URI).withValueBackReference(Data.RAW_CONTACT_ID, 0).withValue(Data.MIMETYPE, ContactsContract.CommonDataKinds.Phone.CONTENT_ITEM_TYPE).withValue(ContactsContract.CommonDataKinds.Phone.NUMBER, ph[i].trim()).withValue(ContactsContract.CommonDataKinds.Phone.TYPE, ContactsContract.CommonDataKinds.Phone.TYPE_CUSTOM).build();
            operations.add(op);
        }
        try {
     ...
     * 当前号码是否在该联系人的所有号码中
     *
     * @param oldnumber    该联系人全部号码
     * @param insertnumber 要插入的用,拼接的号码

 ...
 * @filename: net.m56.ckkj.mobile.tourism.util.RsaUtil.java
...
//        Log.e("MainActivity","公钥加密耗时 cost time---->"+(end-start));
        String encryStr=new BASE64Encoder().encode(encryptBytes);
...
//        Log.e("MainActivity","加密后json数据长度 --1-->"+encryStr.length());
        ...
        try {
            decryptBytes = decryptByPrivateKeyForSpilt(new BASE64Decoder().decodeBuffer(encryStr),privateKey.getEncoded());
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        Log.e("MainActivity","私钥加密密耗时 cost time---->"+(end-start));
        encryStr=new BASE64Encoder().encode(encryptBytes);
...
//        Log.e("MainActivity","加密后json数据长度 --2-->"+encryStr.length());
        ...
        start=System.currentTimeMillis();
        try {
            decryptBytes=  decryptByPublicKeyForSpilt(new BASE64Decoder().decodeBuffer(encryStr),publicKey.getEncoded());
        } catch (Exception e) {
            e.printStackTrace();
        }
